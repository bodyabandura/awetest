...
export const SubAccordion: React.FC<Props> = ({
  toggleAccordion,
  openAccordions,
  mock,
}) => {
  ...
  const handleButtonClick = (key: string, type: "failed" | "all") => {
    setActiveButtons((prev) => ({
      ...prev,
      [key]: type === "failed" ? true : false,
    }));
  };
  ...
  return (
    <div>
      {mock.logs[0].data_mismatch
        .slice(0, 6)
        .map((item: any, index: number) => {
          const key = `subAccordion${index + 1}`;

          const regex =
            /"id"=>\"(\d+)\",.*?"user_id"=>\"(\d+)\",.*?"access"=>\"(\w+)\"/;
              ...
              >
                <div className="flex items-center justify-between gap-1">
                  ...
                  <p>
                    ...
                    <span className="bg-gray-200 rounded-lg py-1 px-2">
                      INDEX {index + 1}
                    </span>{" "}
                    for:{" "}
                    <span className="bg-red-200 text-red-700 rounded-lg py-1 px-2 ml-2">
                      ID
                    </span>
                    <span className="bg-red-200 text-red-700 rounded-lg py-1 px-2 ml-2">
                      USER_ID
                ...
                {openAccordions[key] && (
                  <div className="flex mt-4 relative">
                    ...
                    <div className="flex ml-4">
                      <button
                        className={`py-2 px-4 text-gray-700 border border-gray-300 rounded-l-lg focus:outline-none ${
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          handleButtonClick(key, "failed");
                        }}
                      >
                        Show Failed Only
                      </button>
                      <button
                        className={`py-2 px-4 text-gray-700 border border-gray-300 rounded-r-lg focus:outline-none ${
